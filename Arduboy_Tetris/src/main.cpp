...
void loop()
{
  ...
  if ((frames - framesPrev) > (10U - level))
  {
    ...
    Sprites::drawErase(tetX , tetY , tetriminios [p + rotatePrev] , 0);

    if ( !Collision() )                      // If there is
    {
      if ( moveX != 0)
      {
        tetX -= moveX ;

        if ( Collision())tetX -= moveX ;
        moveX = 0;
      }
    ...
    {
      Sprites::drawSelfMasked(tetX, tetY , tetriminios [p + rotatePrev] , 0);
    ...
    Sprites::drawSelfMasked(tetX , tetY , tetriminios [ p + rotate], 0);
